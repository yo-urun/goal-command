...
export async function callJudge(goal, lastResponse, subgoals = [], config = {}) {
  ...
  // Call the OpenClaw gateway's local LLM API
  // Fail-open: any error → continue (judge broken ≠ progress stuck)
  try {
    const port = process.env.OPENCLAW_PORT || 3000;
    const model = cfg.judgeModel || process.env.OPENCLAW_JUDGE_MODEL || 'default';
    const timeout = (cfg.judgeTimeout || 30) * 1000;

    const response = await fetch(`http://localhost:${port}/api/v1/chat/completions`, {
      method: 'POST',
      ...
      body: JSON.stringify({
        ...
        max_tokens: cfg.judgeMaxTokens || 4096,
        temperature: 0,
      }),
    ...
    if (!response.ok) {
      // API error → fail-open, continue
      return { verdict: 'continue', reason: `judge API error: ${response.status} ${response.statusText}`, parse_failed: false };
    }
    ...
    if (parsed.parse_failed) {
      // Judge returned non-JSON — continue but flag as parse failure
    ...
    // Network error, timeout, or any other exception → fail-open
